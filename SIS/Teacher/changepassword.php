...
<body>
    ...
    <form action="" method="post" enctype="multipart/form-data" id="doc-upload">
            <div class="form-group">
                ...
                <input type="password" name="Old" required id="input-old" class="form-control">
            </div>
            <div class="form-group">
                <label for="input-des">Confirm Password</label>
                <input type="password" name="New" id="input-new" required class='form-control'>
                <span id="err"></span>
            </div>
            <br>
     ...
     if(isset($_POST['Change']))
    {
        //print_r($_POST);
        $pass=$_POST['New'];
        $qry="UPDATE teacher set Password='$pass' where TID='$teacher'";
        if(mysqli_query($conn,$qry))
        {
            echo "<script>alert('Password Changed');</script>";  
            echo "<script>window.location.href='../Login.php?Loger=Teacher';</script>"; 
        }
